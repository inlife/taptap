...
"click"===type?rmv.call(layer,type,callback.hijacked||callback,capture):rmv.call(layer,type,callback,capture)},layer.addEventListener=function(type,callback,capture){var adv=Node.prototype.addEventListener;"click"===type?adv.call(layer,type,callback.hijacked||(callback.hijacked=function(event){event.propagationStopped||callback(event)}),capture):adv.call(layer,type,callback,capture)}),"function"==typeof layer.onclick&&(oldOnClick=layer.onclick,layer.addEventListener("click",function(event){oldOnClick(event)},!1),layer.onclick=null)}}var deviceIsWindowsPhone=navigator.userAgent.indexOf("Windows Phone")>=0,deviceIsAndroid=navigator.userAgent.indexOf("Android")>0&&!deviceIsWindowsPhone,deviceIsIOS=/iP(ad|hone|od)/.test(navigator.userAgent)&&!deviceIsWindowsPhone,deviceIsIOS4=deviceIsIOS&&/OS 4_\d(_\d)?/.test(navigator.userAgent),deviceIsIOSWithBadTarget=deviceIsIOS&&/OS [6-7]_\d/.test(navigator.userAgent),deviceIsBlackBerry10=navigator.userAgent.indexOf("BB10")>0;FastClick.prototype.needsClick=function(target){switch(target.nodeName.toLowerCase()){case"button":case"select":case"textarea":if(target.disabled)return!0;break;case"input":if(deviceIsIOS&&"file"===target.type||target.disabled)return!0;break;case"label":case"iframe":case"video":return!0}return/\bneedsclick\b/.test(target.className)},FastClick.prototype.needsFocus=function(target){switch(target.nodeName.toLowerCase()){case"textarea":return!0;case"select":return!deviceIsAndroid;case"input":switch(target.type){case"button":case"checkbox":case"file":case"image":case"radio":case"submit":return!1}return!target.disabled&&!target.readOnly;default:return/\bneedsfocus\b/.test(target.className)}},FastClick.prototype.sendClick=function(targetElement,event){var clickEvent,touch;document.activeElement&&document.activeElement!==targetElement&&document.activeElement.blur(),touch=event.changedTouches[0],clickEvent=document.createEvent("MouseEvents"),clickEvent.initMouseEvent(this.determineEventType(targetElement),!0,!0,window,1,touch.screenX,touch.screenY,touch.clientX,touch.clientY,!1,!1,!1,!1,0,null),clickEvent.forwardedTouchEvent=!0,targetElement.dispatchEvent(clickEvent)},FastClick.prototype.determineEventType=function(targetElement){return deviceIsAndroid&&"select"===targetElement.tagName.toLowerCase()?"mousedown":"click"},FastClick.prototype.focus=function(targetElement){var length;deviceIsIOS&&targetElement.setSelectionRange&&0!==targetElement.type.indexOf("date")&&"time"!==targetElement.type&&"month"!==targetElement.type?(length=targetElement.value.length,targetElement.setSelectionRange(length,length)):targetElement.focus()},FastClick.prototype.updateScrollParent=function(targetElement){var scrollParent,parentElement;if(scrollParent=targetElement.fastClickScrollParent,!scrollParent||!scrollParent.contains(targetElement)){parentElement=targetElement;do{if(parentElement.scrollHeight>parentElement.offsetHeight){scrollParent=parentElement,targetElement.fastClickScrollParent=parentElement;break}parentElement=parentElement.parentElement}while(parentElement)}scrollParent&&(scrollParent.fastClickLastScrollTop=scrollParent.scrollTop)},FastClick.prototype.getTargetElementFromEventTarget=function(eventTarget){return eventTarget.nodeType===Node.TEXT_NODE?eventTarget.parentNode:eventTarget},FastClick.prototype.onTouchStart=function(event){var targetElement,touch,selection;if(event.targetTouches.length>1)return!0;if(targetElement=this.getTargetElementFromEventTarget(event.target),touch=event.targetTouches[0],deviceIsIOS){if(selection=window.getSelection(),selection.rangeCount&&!selection.isCollapsed)return!0;if(!deviceIsIOS4){if(touch.identifier&&touch.identifier===this.lastTouchIdentifier)return event.preventDefault(),!1;this.lastTouchIdentifier=touch.identifier,this.updateScrollParent(targetElement)}}return this.trackingClick=!0,this.trackingClickStart=event.timeStamp,this.targetElement=targetElement,this.touchStartX=touch.pageX,this.touchStartY=touch.pageY,event.timeStamp-this.lastClickTime<this.tapDelay&&event.preventDefault(),!0},FastClick.prototype.touchHasMoved=function(event){var touch=event.changedTouches[0],boundary=this.touchBoundary;return Math.abs(touch.pageX-this.touchStartX)>boundary||Math.abs(touch.pageY-this.touchStartY)>boundary?!0:!1},FastClick.prototype.onTouchMove=function(event){return this.trackingClick?((this.targetElement!==this.getTargetElementFromEventTarget(event.target)||this.touchHasMoved(event))&&(this.trackingClick=!1,this.targetElement=null),!0):!0},FastClick.prototype.findControl=function(labelElement){return void 0!==labelElement.control?labelElement.control:labelElement.htmlFor?document.getElementById(labelElement.htmlFor):labelElement.querySelector("button, input:not([type=hidden]), keygen, meter, output, progress, select, textarea")},FastClick.prototype.onTouchEnd=function(event){var forElement,trackingClickStart,targetTagName,scrollParent,touch,targetElement=this.targetElement;if(!this.trackingClick)return!0;if(event.timeStamp-this.lastClickTime<this.tapDelay)return this.cancelNextClick=!0,!0;if(event.timeStamp-this.trackingClickStart>this.tapTimeout)return!0;if(this.cancelNextClick=!1,this.lastClickTime=event.timeStamp,trackingClickStart=this.trackingClickStart,this.trackingClick=!1,this.trackingClickStart=0,deviceIsIOSWithBadTarget&&(touch=event.changedTouches[0],targetElement=document.elementFromPoint(touch.pageX-window.pageXOffset,touch.pageY-window.pageYOffset)||targetElement,targetElement.fastClickScrollParent=this.targetElement.fastClickScrollParent),targetTagName=targetElement.tagName.toLowerCase(),"label"===targetTagName){if(forElement=this.findControl(targetElement)){if(this.focus(targetElement),deviceIsAndroid)return!1;targetElement=forElement}}else if(this.needsFocus(targetElement))return event.timeStamp-trackingClickStart>100||deviceIsIOS&&window.top!==window&&"input"===targetTagName?(this.targetElement=null,!1):(this.focus(targetElement),this.sendClick(targetElement,event),deviceIsIOS&&"select"===targetTagName||(this.targetElement=null,event.preventDefault()),!1);return deviceIsIOS&&!deviceIsIOS4&&(scrollParent=targetElement.fastClickScrollParent,scrollParent&&scrollParent.fastClickLastScrollTop!==scrollParent.scrollTop)?!0:(this.needsClick(targetElement)||(event.preventDefault(),this.sendClick(targetElement,event)),!1)},FastClick.prototype.onTouchCancel=function(){this.trackingClick=!1,this.targetElement=null},FastClick.prototype.onMouse=function(event){return this.targetElement?event.forwardedTouchEvent?!0:event.cancelable&&(!this.needsClick(this.targetElement)||this.cancelNextClick)?(event.stopImmediatePropagation?event.stopImmediatePropagation():event.propagationStopped=!0,event.stopPropagation(),event.preventDefault(),!1):!0:!0},FastClick.prototype.onClick=function(event){var permitted;return this.trackingClick?(this.targetElement=null,this.trackingClick=!1,!0):"submit"===event.target.type&&0===event.detail?!0:(permitted=this.onMouse(event),permitted||(this.targetElement=null),permitted)},FastClick.prototype.destroy=function(){var layer=this.layer;deviceIsAndroid&&(layer.removeEventListener("mouseover",this.onMouse,!0),layer.removeEventListener("mousedown",this.onMouse,!0),layer.removeEventListener("mouseup",this.onMouse,!0)),layer.removeEventListener("click",this.onClick,!0),layer.removeEventListener("touchstart",this.onTouchStart,!1),layer.removeEventListener("touchmove",this.onTouchMove,!1),layer.removeEventListener("touchend",this.onTouchEnd,!1),layer.removeEventListener("touchcancel",this.onTouchCancel,!1)},FastClick.notNeeded=function(layer){var metaViewport,chromeVersion,blackberryVersion,firefoxVersion;if("undefined"==typeof window.ontouchstart)return!0;if(chromeVersion=+(/Chrome\/([0-9]+)/.exec(navigator.userAgent)||[,0])[1]){if(!deviceIsAndroid)return!0;if(metaViewport=document.querySelector("meta[name=viewport]")){if(-1!==metaViewport.content.indexOf("user-scalable=no"))return!0;if(chromeVersion>31&&document.documentElement.scrollWidth<=window.outerWidth)return!0}}if(deviceIsBlackBerry10&&(blackberryVersion=navigator.userAgent.match(/Version\/([0-9]*)\.([0-9]*)/),blackberryVersion[1]>=10&&blackberryVersion[2]>=3&&(metaViewport=document.querySelector("meta[name=viewport]")))){if(-1!==metaViewport.content.indexOf("user-scalable=no"))return!0;if(document.documentElement.scrollWidth<=window.outerWidth)return!0}return"none"===layer.style.msTouchAction||"manipulation"===layer.style.touchAction?!0:(firefoxVersion=+(/Firefox\/([0-9]+)/.exec(navigator.userAgent)||[,0])[1],firefoxVersion>=27&&(metaViewport=document.querySelector("meta[name=viewport]"),metaViewport&&(-1!==metaViewport.content.indexOf("user-scalable=no")||document.documentElement.scrollWidth<=window.outerWidth))?!0:"none"===layer.style.touchAction||"manipulation"===layer.style.touchAction?!0:!1)},FastClick.attach=function(layer,options){return new FastClick(layer,options)},"function"==typeof define&&"object"==typeof define.amd&&define.amd?define(function(){return FastClick}):"undefined"!=typeof module&&module.exports?(module.exports=FastClick.attach,module.exports.FastClick=FastClick):window.FastClick=FastClick}();var store=function(){"use strict";var store=function(key,value){function createCookie(key,value,exp){var date=new Date;date.setTime(date.getTime()+24*exp*60*60*1e3);var expires="; expires="+date.toGMTString();document.cookie=key+"="+value+expires+"; path=/"}function readCookie(key){for(var nameEQ=key+"=",ca=document.cookie.split(";"),i=0,max=ca.length;max>i;i++){for(var c=ca[i];" "===c.charAt(0);)c=c.substring(1,c.length);if(0===c.indexOf(nameEQ))return c.substring(nameEQ.length,c.length)}return null}var lsSupport=!1;if(localStorage&&(lsSupport=!0),"undefined"!=typeof value&&null!==value&&("object"==typeof value&&(value=JSON.stringify(value)),lsSupport?localStorage.setItem(key,value):createCookie(key,value,30)),"undefined"==typeof value){data=lsSupport?localStorage.getItem(key):readCookie(key);try{data=JSON.parse(data)}catch(e){data=data}return data}null===value&&(lsSupport?localStorage.removeItem(key):createCookie(key,"",-1))};return store}(),Cell=function(){"use strict";var Cell=function(id,game,state){this.id=id?id:null,this.entity=id?$("#"+id):null,this.state=state?state:Math.round(Math.random()),this.connected=[],this.isMagic=!1,this.initialize(game)};return Cell.prototype.initialize=function(game){var self=this;this.state&&this.entity.addClass("on"),this.entity.bind("click",function(){console.log(self.isMagic),self.toggle(),self.toggleConnected(),game.trigger("cell:toggle")})},Cell.prototype.connect=function(cell,backward){return backward?cell.connect(this):void this.connected.push(cell)},Cell.prototype.toggle=function(){this.state=this.state?0:1,this.entity.toggleClass("on")},Cell.prototype.toggleConnected=function(){if(this.isMagic)this.state!==this.connected[0].state&&this.connected[0].toggle();else for(var i=this.connected.length-1;i>=0;i--)this.connected[i].toggle()},Cell.prototype.remove=function(){for(var i=this.connected.length-1;i>=0;i--)delete this.connected[i];this.entity.remove()},Cell.prototype.setMagic=function(isMagic){return this.isMagic=isMagic,this},Cell.prototype.setState=function(state){return this.state=state,this.state&&!this.entity.hasClass("on")?this.entity.toggleClass("on"):!this.state&&this.entity.hasClass("on")&&this.entity.toggleClass("on"),this},Cell}(),Game=function(){"use strict";var Game=function(selectors,size,percent){return!selectors||!selectors.field||selectors.field.length<1?console.error("Please, provide correct selector object as first parameter."):(this.selectors=selectors,this.entity=selectors.field?$(selectors.field):null,this.size=size?size:4,this.percent=percent?percent:45,this.cells=[],this.events=[],this.registerEvents(),this.entity.data("size")!=this.size&&this.regenerate(),this.bindCells(),void this.resize())};return Game.prototype.registerEvents=function(){var self=this;self.on("cell:toggle",function(){for(var states=0,i=self.cells.length-1;i>=0;i--)states+=self.cells[i].state;(0===states||states===self.size*self.size)&&self.trigger("game:complete")}),self.on("game:complete",function(){console.log("you won! now it will be restarted, and BIGGER!!"),setTimeout(function(){self.restart()},1e3)})},Game.prototype.regenerate=function(){this.removeCells(),this.entity.attr("data-size",this.size);for(var x=1;x<=this.size;x++){for(var line=$('<div class="'+this.selectors.line.replace(/[\#\.]/i,"")+'">'),y=1;y<=this.size;y++)line.append($('<div class="'+this.selectors.cell.replace(/[\#\.]/i,"")+'" id="cell_'+x+"_"+y+'">'));this.entity.append(line)}},Game.prototype.restart=function(size){this.setSize(size?size:this.size,!0)},Game.prototype.setSize=function(size,force){(size&&size!=this.size||force===!0)&&(this.size=size,this.regenerate(),this.bindCells(),this.resize())},Game.prototype.resize=function(){this.entity.css("width",this.getFieldSize()),this.entity.css("height",this.getFieldSize()),$(".copyright").css("width",this.getFieldSize()),$(".time").css("width",this.getFieldSize()),$(".field").click(function(event){$("main").removeClass("start")}),window.innerWidth<this.getFieldSizeinPixelsStatic()&&$("body").css("font-size",window.innerWidth/this.size)},Game.prototype.getFieldSize=function(){return this.size*C_SIZE_SPACE+"em"},Game.prototype.getFieldSizeinPixelsStatic=function(){return C_BASIC_SCALE*this.size},Game.prototype.addCell=function(cell){this.cells.push(cell)},Game.prototype.getRandomCell=function(){return this.cells[Math.floor(Math.random()*this.cells.length)]},Game.prototype.removeCells=function(){for(var i=this.cells.length-1;i>=0;i--)this.cells[i].remove(),delete this.cells[i];this.cells.length=0,this.entity.children().remove()},Game.prototype.on=function(name,callback){this.events[name]?this.events[name].push(callback):this.events[name]=[callback]},Game.prototype.trigger=function(name,event){if(this.events[name])for(var i=this.events[name].length-1;i>=0;i--)this.events[name][i](event)},Game.prototype.save=function(name){var self=this;name=name?name:"_sgp1_save_1";for(var savedata={time:Date.now(),game:{size:self.size,cells:[]}},i=this.cells.length-1;i>=0;i--){for(var cell=this.cells[i],connected=[],j=cell.connected.length-1;j>=0;j--){var subcell=cell.connected[j];connected.push(subcell.id)}savedata.game.cells.push({id:cell.id,state:cell.state,isMagic:cell.isMagic,connected:connected})}store(name,savedata),self.trigger("game:save")},Game.prototype.load=function(name){var self=this;name=name?name:"_sgp1_save_1";var data=store(name);this.setSize(data.game.size);for(var i=this.cells.length-1,j=0;i>=0;i--,j++){var ccell=this.cells[j],scell=data.game.cells[i];ccell.setState(scell.state),ccell.setMagic(scell.isMagic?!0:!1),ccell.connected.length=0;for(var k=scell.connected.length-1;k>=0;k--)ccell.connect(this.getCellById(scell.connected[k]))}self.trigger("game:load")},Game.prototype.getCellById=function(id){for(var i=this.cells.length-1;i>=0;i--)if(this.cells[i].id==id)return this.cells[i];return null},Game.prototype.bindCells=function(){var self=this;$.each($(self.selectors.cell),function(){self.addCell(new Cell($(this).attr("id"),self))}),self.connectCells_v2(),self.trigger("game:generated")},Game.prototype.connectCells_v1=function(){for(var self=this,count=this.size*this.size,part=Math.floor(count/100*this.percent),pool={},getRandomCellFromPull=function(compareCell){for(var i=0;9999>i;i++){var cell=self.getRandomCell();if(pool[cell.id]===!0){if(compareCell&&cell.id===compareCell.id)continue;return pool[cell.id]=!1,cell}}},i=0;count>i;i++)pool[this.cells[i].id]=!0;for(var pcell=getRandomCellFromPull(),i=0;part>i;i++){var cell=getRandomCellFromPull(pcell);pcell.connect(cell,!0),pcell=cell}for(var i=0;part>i;i++){for(var cell=null,j=0;2>j;j++,i++)cell=getRandomCellFromPull(pcell),pcell.connect(cell);pcell=cell}},Game.prototype.connectCells_v2=function(){var game=this,size=this.size,field=size*size,pool={data:{},getRandomCell:function(compareCell){for(var self=pool,i=0;9999>i;i++){var cell=game.getRandomCell();if(self.data[cell.id]===!0){if(compareCell&&cell.id===compareCell.id)continue;return self.data[cell.id]=!1,cell}}},getFreeCount:function(){var self=pool,count=0;for(var id in self.data)count+=self.data[id]?1:0;return count},create:function(){for(var self=pool,i=0;field>i;i++)self.data[game.cells[i].id]=!0}},cycles={minimum:3,maximum:6,limit:Math.ceil(field/2),count:0,data:[],generate:function(){var self=cycles,combinations=[],limit=self.limit;!function(combinations){var rec_combinate=function(prev){for(var i=self.minimum;i<=self.maximum;i++){var curr={members:[],sum:0};if(prev)var lprev=prev;else var lprev=curr;lprev.sum+i<=limit&&(curr.members=lprev.members.slice(),curr.members.push(i),curr.sum=lprev.sum+i,combinations.push(curr.members),rec_combinate(curr))}};return rec_combinate(null)}(combinations);var result=combinations[Math.floor(Math.random()*combinations.length)];self.count=result.length;for(var i=result.length-1;i>=0;i--){for(var cycle_size=result[i],cells=[],j=0;cycle_size>j;j++){cells.push(pool.getRandomCell());var cell=cells[j];j>0&&cells[j-1].connect(cell),j===cycle_size-1&&cells[j].connect(cells[0])}self.data.push(cells)}console.log(result)},getRandom:function(compare){for(var self=cycles,k=0;9999>k;k++){if(!self.data.length)return null;if(1===self.data.length)return self.data[0];var item=self.data[Math.floor(Math.random()*self.data.length)];if(!compare||item!=compare)return item}},getRandomCellFromCycle:function(cycle){return cycle[Math.floor(Math.random()*cycle.length)]},link:function(){var self=cycles,linkSequence=function(maxsize,input,output){var rsize=Math.max(Math.ceil(Math.random()*maxsize+3),Math.ceil(Math.random()*maxsize+3));if(rsize>pool.getFreeCount())return linkSequence(maxsize-1,input,output);for(var prev=input,i=0;rsize>i;i++){var curr=pool.getRandomCell();prev.connect(curr),prev=curr}prev.connect(output)},starting=pool.getRandomCell();starting.setMagic(!0);var maxsize=Math.floor(pool.getFreeCount()/self.count),base=self.data[0],last=base;linkSequence(maxsize,starting,self.getRandomCellFromCycle(base));for(var i=1;i<self.count;i++){var rnd=2*Math.random()>1?!0:!1,curr=self.data[i];linkSequence(maxsize,self.getRandomCellFromCycle(base),self.getRandomCellFromCycle(curr)),(base.length<4||!rnd||base!=last)&&(last=curr)}for(var chain=self.getRandomCellFromCycle(last),i=pool.getFreeCount()-1;i>=0;i--){var curr=pool.getRandomCell();chain.connect(curr),chain=curr}}};pool.create(),cycles.generate(),cycles.link(),console.log(pool.getFreeCount())},Game}(),dbg=null,C_SIZE_SPACE=.98,C_BASIC_SCALE=100;$(document).ready(function(){var myGame=new Game({field:".field",line:".line",cell:".cell"},4);$(window).resize(function(){myGame.resize()}),dbg=myGame}),window.addEventListener("touchmove",function(event){event.target.classList.contains("scrollable")||event.preventDefault()},!1),window.addEventListener("load",function(){new FastClick(document.body)},!1);